...
Keep edits aligned with the current repository state, even when the project has changed since the last task.

## Workflow

1. Re-read the files that define the current behavior before making assumptions.
2. Prefer source code and tests over memory, comments, or stale examples.
3. If documentation disagrees with code, treat the code as the source of truth and report the mismatch.
4. After editing, run or review the most relevant tests for the changed area.
5. Summarize any behavior changes clearly so the user can verify them quickly.
